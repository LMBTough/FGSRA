...
def normalize(grad, opt=2):
    if opt == 0:
        ...
        try:
            ...
            square = torch.sum(torch.square(grad), dim=(1, 2), keepdim=True)
...
class DANAA(Attack):
    def __init__(self, model, eps=8/255,
                 alpha=2/255, steps=10,ieps=16/255,method="DIPIBIM",model_learning_rate=0.0001,train_steps=[0,2,4,6,8]):
        ...
        if model[1].__class__.__name__ == "Inception3":
            ...
            for name, module in self.model.named_modules():
                if name == "1.conv2d_4a.relu":
                    module.register_forward_hook(hook_feature)
                    break
        elif model[1].__class__.__name__ == "MaxVit":
            for i,(name, module) in enumerate(self.model.named_modules()):
                if i == 25:
                    module.register_forward_hook(hook_feature)
                    break
        elif model[1].__class__.__name__ == "VisionTransformer":
            for i,(name, module) in enumerate(self.model.named_modules()):
                if i == 25:
                    module.register_forward_hook(hook_feature)
                    break
    ...
    def forward(self, x, y, save_func,save_steps,output_dir):
