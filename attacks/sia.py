...
class Attack(object):
    ...
    """
    def __init__(self, attack, model, eps, targeted, random_start, norm, loss, model_learning_rate=0.0001,train_steps=[0,2,4,6,8],device=None):
        """
    ...
    def forward(self, data, label, save_func,save_steps,output_dir, **kwargs):
    # def forward(self, data, label, **kwargs):
        ...
        for step in range(self.steps):
            if step in self.train_steps:
                ...
                optimizer.step()
            # Obtain the output
            logits = model(self.transform(data+delta, momentum=momentum))
            # Calculate the loss
        ...
        # save_func(images=delta,next_grad=grad,output_dir=output_dir)
        return x.detach()
...
class MIFGSM(Attack):
    ...
    def __init__(self, model, eps=16/255, alpha=2/255, steps=10, decay=1., targeted=False, random_start=False, 
                norm='linfty', loss='crossentropy', device=None, attack='MI-FGSM', model_learning_rate=0.0001,train_steps=[0,2,4,6,8], **kwargs):
...
class SIA(MIFGSM):
    ...
    def __init__(self, model, eps=16/255, alpha=2/255, steps=10, decay=1., num_copies=20, num_block=3, targeted=False, random_start=False, 
                norm='linfty', loss='crossentropy', device=None, attack='SIM', model_learning_rate=0.0001,train_steps=[0,2,4,6,8], **kwargs):
        super().__init__(model, eps, alpha, steps, decay, targeted, random_start, norm, loss, device, attack, model_learning_rate=0.0001,train_steps=[0,2,4,6,8], **kwargs)
