...
class TIFGSM(Attack):
    ...
    def __init__(self, model, eps=8/255, alpha=2/255, steps=10, decay=0.0, kernel_name='gaussian',
                 len_kernel=15, nsig=3, resize_rate=0.9, diversity_prob=0.5, random_start=False, model_learning_rate=0.0001,train_steps=[0,2,4,6,8]):
    ...
    def forward(self, images, labels, save_func,save_steps,output_dir):
        ...
        success_steps = np.inf * np.ones(len(images))
    ...
    def input_diversity(self, x):
        ...
        if img_size == 224:
            padded = F.interpolate(padded, size=[img_size, img_size], mode='bilinear', align_corners=False) # resize to original size
